...
from core.types import Citation, Grade, MatchEvidence, Verdict
...
def match(
    ...
    *,
    ...
    all_verdicts.sort(key=lambda v: (v.citation.seq, v.citation.quote))
    return all_verdicts
...
def _stage_diff(
    ...
    for c, ev_exact, ev_fuzzy in zip(citations, exact, fuzzy):
        ...
        if ev is None:
            # 无候选 → 直接 D；LLM prompt 的硬约束保证它对空 matched_passage 也只能给 NOT_FOUND，
            # 没必要再调一次。
            verdicts.append(Verdict(citation=c, grade="D", evidence=None,
                                    issues=["未在任何参考文献中找到"],
                                    needs_llm=False, confidence=0.0))
            continue
